...
import { loadStation, removeStation, setStationOrder } from '../../store/station/station.actions'
...
export function SidebarList({ stations = [], user, isCollapsed }) {
  ...
  useEffect(() => {
    ...
    const mismatch = orderedIds.length !== actualIds.length || !actualIds.every(id => orderedIds.includes(id))
  ...
  async function onDeleteStation(station) {
    try {
      ...
      const msgImg = station.imgUrl
        ? station.imgUrl
        : `https://res.cloudinary.com/dpoa9lual/image/upload/v1724570942/Spotify_playlist_photo_yjeurq.png`

      showSuccessMsg('Removed from Your Library', msgImg)
    } catch {
      showErrorMsg('Could not remove station')
    }
  }

  async function onRemoveFromLibrary(station) {
    try {
      const updatedLikedStationIds = user.likedStationIds.filter(id => id !== station._id)
      ...
      const newOrder = stationOrder.filter(id => id !== station._id)
      setStationOrder(newOrder)
...
function DraggableStation({ station, index, moveStation, onContextMenu, ...rest }) {
  ...
  return (
    <SidebarPreview station={station} setDragRef={setDragRef} isOver={isOver} onContextMenu={onContextMenu} {...rest} />
  )
}
